...
- **[2026-07-13] wiki:update** — Added `docs/modernization-planning/prd.md`: a lightweight 1-day execution PRD (problem, mission, current/target state, scope, success criteria, rubric alignment, flows to preserve, constraints, risks, 4-person model, day milestones, demo checklist, open decisions). Grounded in the wiki + planning docs; separates env prerequisites from the real CS0234 defect (eliminated by the WCF→minimal API target). See [[log]].
- **[2026-07-13] wiki:update** — Added `docs/modernization-planning/hackathon-brief.md` (mission, current/target state, flows to preserve, success criteria, rubric priorities, risks, strategy, team focus, first actions, open questions). See [[log]].
- **[2026-07-13] wiki:update** — Added `docs/modernization-planning/plan.md` + `docs/modernization-planning/backlog.md` (epics E1–E13, stories, acceptance criteria, rubric mapping). Ignored BMAD tooling in `.gitignore` (`.agents/`, `.claude/`, `.github/agents/`, `_bmad/`, `_bmad-output/`); teammates reinstall via `npx bmad-method install`. See [[log]].
